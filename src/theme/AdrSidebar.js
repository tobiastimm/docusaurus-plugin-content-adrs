...
import styles from './adr-sidebar.module.css'

const MOBILE_TOGGLE_SIZE = 24

function AdrSidebar(props) {
  const [showResponsiveSidebar, setShowResponsiveSidebar] = useState(false)
  const { adrsMetadata, sidebar } = props

  if (!sidebar) {
    return null
  }

  const thisSidebar = adrsMetadata.adrsSidebars[sidebar]
  ...
  const convertDocLink = item => {
    ...
    const linkMetadata = adrsMetadata.adrs[linkID]
